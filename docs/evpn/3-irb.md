...
* If a switch decides to advertise a host IP address in a MAC-IP route, it could originate a single MAC-IP route with both addresses, or advertise (like Arista EOS) two MAC-IP routes (a MAC-only route and a MAC+IP route). RFC 7432 [allows both behaviors](https://datatracker.ietf.org/doc/html/rfc7432#section-10) and describes the scenario where both routes are used when MAC learning is separate from ARP snooping.
